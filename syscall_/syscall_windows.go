...
//	errorspkg "errors"
	//"internal/oserror"
	"github.com/hacdias/webdav/v3/internal_/race_"
//	"runtime"
//	"sync"
	//"unicode/utf16"
	"unsafe"
...
func Read(host string, fd syscall.Handle, p []byte) (n int, err error) {
	var done uint32
	e := ReadFile_(host, fd, p, &done, nil)
	if e != nil {
		if e == syscall.ERROR_BROKEN_PIPE {
	...
	if race_.Enabled {
		if done > 0 {
			race_.WriteRange(unsafe.Pointer(&p[0]), int(done))
		}
		race_.Acquire(unsafe.Pointer(&ioSync))
	}
...
func setFilePointerEx_(host string, handle syscall.Handle, distToMove int64, newFilePointer *int64, whence uint32) error {
	...
	// case "386":
		// distToMove is a LARGE_INTEGER:
		// https://msdn.microsoft.com/en-us/library/windows/desktop/aa383713(v=vs.85).aspx
		_, _, e1 = syscall.Syscall6(GetProc("_SetFilePointerEx"), 6, uintptr(unsafe.Pointer(serverp)), uintptr(handle), uintptr(distToMove), uintptr(distToMove>>32), uintptr(unsafe.Pointer(newFilePointer)), uintptr(whence))
	// case "arm":
	// 	// distToMove must be 8-byte aligned per ARM calling convention
	// 	// https://msdn.microsoft.com/en-us/library/dn736986.aspx#Anchor_7
	// 	_, _, e1 = Syscall6(procSetFilePointerEx.Addr(), 6, uintptr(handle), 0, uintptr(distToMove), uintptr(distToMove>>32), uintptr(unsafe.Pointer(newFilePointer)), uintptr(whence))
...
func Seek_(host string, fd syscall.Handle, offset int64, whence int) (newoffset int64, err error) {
	var w uint32
	switch whence {
	case 0:
		w = FILE_BEGIN
...
func FindFirstFile(name *uint16, data *syscall.Win32finddata) (handle syscall.Handle, err error) {
...
func FindFirstFile_Close(name *uint16, data *syscall.Win32finddata) (err error) {
	// NOTE(rsc): The Win32finddata struct is wrong for the system call:
	// the two paths are each one uint16 short. Use the correct struct,
	// a win32finddata1, and then copy the results out.
	// There is no loss of expressivity here, because the final
