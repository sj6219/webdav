...
func (fd *FD) destroy() error {
	...
	case kindDir:
		err = syscall_.FindClose_(fd.Host, fd.Sysfd)
	default:
		err = syscall_.CloseHandle_(fd.Host, fd.Sysfd)
	}
...
func (fd *FD) Close() error {
	...
	if fd.kind == kindPipe {
		syscall_.CancelIoEx_(fd.Host, fd.Sysfd, nil)
	}
...
func (fd *FD) Read(buf []byte) (int, error) {
	...
	if fd.isFile {
		...
		default:
			n, err = syscall_.Read(fd.Host, fd.Sysfd, buf)
...
func (fd *FD) Seek(offset int64, whence int) (int64, error) {
	...
	fd.l.Lock()
	defer fd.l.Unlock()
	return syscall_.Seek_(fd.Host, fd.Sysfd, offset, whence)
}
...
func (fd *FD) FindNextFile(data *syscall.Win32finddata) error {
	...
	defer fd.decref()
	return syscall_.FindNextFile_(fd.Host, fd.Sysfd, data)
}
...
func (fd *FD) GetFileType() (uint32, error) {
	...
	defer fd.decref()
	return syscall_.GetFileType_(fd.Host, fd.Sysfd)
}
