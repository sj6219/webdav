...
// 	"errors"
// 	"internal_/poll"
"github.com/hacdias/webdav/v3/internal_/poll_"
...
func newFile(h syscall.Handle, name string, kind string) *File {
	...
	if kind == "file" {
		...
		// 	kind = "console"
		// }
		if t, err := syscall_.GetFileType_(netname.Server, h); err == nil && t == syscall.FILE_TYPE_PIPE {
			kind = "pipe"
		}
	}
...
func openFile(name string, flag int, perm os.FileMode) (file *File, err error) {
	netname := syscall_.Decompose(name)
	r, e := syscall_.Open(fixLongPath(netname.String()), flag|syscall.O_CLOEXEC, syscallMode(perm))
	if e != nil {
		return nil, e
	}
...
func openDir(name string) (file *File, err error) {
	...
	d := new(dirInfo)
	r, e := syscall_.FindFirstFile(maskp, &d.data)
	if e != nil {
		...
		e = syscall_.GetFileAttributesEx(pathp, syscall.GetFileExInfoStandard, (*byte)(unsafe.Pointer(&fa)))
		if e != nil {
			return nil, e
		}
...
func openFileNolog(name string, flag int, perm os.FileMode) (*File, error) {
	if name == "" {
	...
	if errd == nil {
		if flag&os.O_WRONLY != 0 || flag&os.O_RDWR != 0 {
			r.Close()
			return nil, &os.PathError{"open", name, syscall.EISDIR}
		}
		return r, nil
	}
...
func (file *file) close() error {
	...
	if e := file.pfd.Close(); e != nil {
		if e == poll_.ErrFileClosing {
			e = os.ErrClosed
		}
		err = &os.PathError{"close", file.name, e}
	}
