...
func newFileStatFromGetFileInformationByHandle(path string, h syscall.Handle) (fs *fileStat, err error) {
	netname := syscall_.Decompose(path)
	var d syscall.ByHandleFileInformation
	var ti windows_.FILE_ATTRIBUTE_TAG_INFO
	if (netname.Server == "") {
		syscall_.Debug()
// //		var d syscall.ByHandleFileInformation
...
// 		err = windows_.GetFileInformationByHandleEx(h, windows.FileAttributeTagInfo, (*byte)(unsafe.Pointer(&ti)), uint32(unsafe.Sizeof(ti)))
// 		if err != nil {
...
// 				ti.ReparseTag = 0
// 			} else {
// 				return nil, &os.PathError{"GetFileInformationByHandleEx", path, err}
// 			}
// 		}
	} else {
		...
		_, _, err = syscall.Syscall6(syscall_.GetProc("_GetFileInformationByHandle"), 4, uintptr(unsafe.Pointer(serverp)), uintptr(h), uintptr(unsafe.Pointer(&d)), uintptr(unsafe.Pointer(&ti.ReparseTag)), 0, 0)
...
func (fs *fileStat) isSymlink() bool {
	...
	return fs.Reserved0 == syscall.IO_REPARSE_TAG_SYMLINK ||
		fs.Reserved0 == windows_.IO_REPARSE_TAG_MOUNT_POINT
}
...
func (fs *fileStat) saveInfoFromPath(path string) error {
	...
	if !isAbs(fs.path) {
		...
		if err != nil {
			return &os.PathError{"FullPath", path, err}
		}
	}
